...
int ModernMetal::UnserializeState(const IByteChunk& chunk, int startPos)
{
  ...
  // if strncmp (header.Get(), "###ModernMetal###")
  //{
...
void ModernMetal::_NormalizeModelOutput(iplug::sample** buffer, const size_t numChannels, const size_t numFrames)
{
  if (!mModel)
    return;
...
void ModernMetal::_LearnMaxPeak(iplug::sample** inputs, int nFrames, const size_t nChansIn) {
