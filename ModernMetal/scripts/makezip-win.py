...
def main():
    ...
    if not zip:
        installer = "\\build-win\\installer\\ModernMetal Installer.exe"

        if demo:
            installer = "\\build-win\\installer\\ModernMetal Demo Installer.exe"

        files = [
            projectpath + installer,
            projectpath + "\\installer\\changelog.txt",
            projectpath + "\\installer\\known-issues.txt",
            projectpath + "\\manual\\ModernMetal manual.pdf",
        ]
    else:
        files = [
            projectpath
            + "\\build-win\\ModernMetal.vst3\\Contents\\x86_64-win\\ModernMetal.vst3",
            projectpath + "\\build-win\\ModernMetal_x64.exe",
        ]
    ...
    files = [
        projectpath + "\\build-win\\pdbs\\ModernMetal-vst3_x64.pdb",
        projectpath + "\\build-win\\pdbs\\ModernMetal-app_x64.pdb",
    ]
